  ...
  <div class="editScheme">
    <Form :model="formItem" :label-width="120" ref="formItem" label-position="left" :rules="ruleValidate">
      <FormItem label="方案名称：" prop="name">
        <Input v-model="formItem.name" placeholder="请输入" style="width:80%" :queryid="formItem.id"></Input>
      </FormItem>
      <FormItem label="匹配方式：">
      ...
      <FormItem class="what-yes" label="地域关键词：">
        <Tooltip placement="bottom-start" class="editScheme-what">
        ...
        <Button type="info" style="margin: 15px 0" @click="keywordE('areaKeywords')">添加关键词</Button>
      </FormItem>
      ...
      <FormItem class="what-yes" label="事件关键词：">
        ...
        <Input v-model="formItem.eventKeywords" type="textarea" :autosize="{minRows: 2,maxRows: 8}"
               placeholder="请输入..."></Input>
        <Button type="info" style="margin: 15px 0" @click="keywordE('eventKeywords')">添加关键词</Button>
      </FormItem>
      <FormItem class="what-yes" label="排除关键词：">
        ...
        <Input v-model="formItem.excludeKeywords" type="textarea" :autosize="{minRows: 2,maxRows: 8}"
               placeholder="请输入..."></Input>
        <Button type="info" style="margin: 15px 0" @click="keywordE('excludeKeywords')">添加关键词</Button>
      </FormItem>
      <FormItem class="editScheme-button">
        <Button type="primary" @click="submit('formItem')">修改</Button>
        <Button type="ghost" style="margin-left: 8px" @click="cancels">取消</Button>
        <!--<Button type="error" style="margin-left: 8px"  @click="del(formItem.id)">删除</Button>-->
      </FormItem>
    </Form>
    <Modal
      v-model="modal1"
    ...
    methods: {
      ...
      submit: function (name) {
        this.$refs[name].validate((valid) => {
          if (valid) {
            $.post("http://10.100.82.221:8089/programme/update", this.formItem, (data) => {
              if (data.code === 100) {
                this.$Message.info('方案修改成功');
              }
            }, "json");
          } else {
      ...
      cancels: function () {
        ...
        this.formItem = this.list
      },
      //删除方案  未完善  待定
      del(id){
        $.post("http://10.100.82.221:8089/programme/delete", id.toString(), (data) => {
          if (data.code === 100) {
            this.$Message.info('方案删除成功');
          }
        }, "json");
      },
//      关键词的分类
